...
public class PartitionUtils {
    ...
    public static String buildOBPartitionWithDDL(String ddl, List<PartitionInfo> partitions) {
        ...
        String partitionRanges = "PARTITION p%s VALUES LESS THAN(%s)";
        String partitionLists = "PARTITION p%s VALUES IN(%s)";
        ...
        if (rangeFlag) {
            for (int i = 0; i < partitions.size(); i++) {
                ...
                if (rangeInfo != null) {
                    extents.add(String.format(partitionRanges, i, rangeInfo.f1));
                }
            }
        }
        ...
        if (listFlag) {
            ...
            for (PartitionInfo partition : partitions) {
                Tuple2<Integer, String> listPartition =
                        toListPartition(
                                partitionLists,
                                partition.getList(),
                                partition.getPartitionKeyType(),
                                flag);
                flag = listPartition.f0;
                extents.add(listPartition.f1);
            }
        }
    ...
    public static Tuple2<Integer, String> toListPartition(
            String listPartitionTemplate,
            String input,
            List<String> partitionKeyTypes,
            Integer flag) {
        input = input.substring(1, input.length() - 1);
        // Split the strings to get each group
        String[] pairs = input.split("\\),\\s*\\(");
        List<String> partitionInfos = new ArrayList<>();
        // Process each group
        for (String pair : pairs) {
            // Clean up the brackets for each set of head and tail brackets
            ...
            partitionInfos.add(String.format(listPartitionTemplate, flag, resultValue));
            flag++;
        }
        return new Tuple2<>(flag, String.join(",", partitionInfos));
    }
